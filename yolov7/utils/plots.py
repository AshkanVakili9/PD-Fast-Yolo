...
def plot_one_box_PIL(box, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        fontsize = max(round(max(img.size) / 40), 12)
        font_path = 'C:/Users/ASH/Desktop/Maryam/yolov7/utils/Yekan.ttf'
        font = ImageFont.truetype(font_path, fontsize)
        # Use only one value as the position for draw.textbbox
        position = (box[0], box[1] - fontsize)  # Adjust the position as needed
        bbox = draw.textbbox(position, label, font)
